...
async def test_agent_only_offers_its_own_capabilities_to_the_model() -> None:
    registry = _registry_with(
        ("support.faq.search", SideEffect.READ),
        ("settings.profile.get", SideEffect.READ),
        ("settings.preferences.get", SideEffect.READ),
        ("settings.sessions.list", SideEffect.READ),
        ("payments.transfer", SideEffect.MONEY_MOVING),
        ("accounts.list", SideEffect.READ),
    )
    chat = _ScriptedChat([ChatResult(content="hi", tool_calls=[], message={})])
    agent = SupportAgent(chat=chat, capabilities=registry, audit=_noop_audit)
