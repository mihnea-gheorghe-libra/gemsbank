...
def _payment_to_bson(payment: Payment) -> dict[str, Any]:
    return {
        ...
        "targetCurrency": payment.target_currency,
        "targetAmountMinorUnits": payment.target_amount_minor,
        "conversionRateMicro": payment.conversion_rate_micro,
...
def _payment_from_bson(raw: dict[str, Any]) -> Payment:
    return Payment(
        ...
        amount_minor=raw["amountMinorUnits"],
        currency=raw["currency"],
        target_currency=raw.get("targetCurrency"),
        target_amount_minor=raw.get("targetAmountMinorUnits"),
        conversion_rate_micro=raw.get("conversionRateMicro"),
        reference=raw["reference"],
        category=raw["category"],
