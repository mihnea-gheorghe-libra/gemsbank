...
def normalise_email(raw: str) -> str:
    ...
    if not EMAIL_PATTERN.match(candidate):
        raise ValidationError("That does not look like an email address.", details={"field": "email"})
    return candidate
...
def validate_pin(raw: str, confirmation: str) -> str:
    ...
    if len(set(raw)) == 1:
        raise ValidationError("The PIN cannot be the same digit repeated.", details={"field": "pin"})
    if any(raw in sequence for sequence in _SEQUENTIAL):
        raise ValidationError("The PIN cannot be a run of consecutive digits.", details={"field": "pin"})
    return raw
